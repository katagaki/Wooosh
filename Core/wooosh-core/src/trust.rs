//! Pinned peer public keys (PROTOCOL.md §4.5), a JSON map keyed by base64
//! pubkey. `last_addr` lets `connect_to` enforce a pin when the shell passes no
//! expected key (DESIGN.md §4).
...
pub struct TrustEntry {
    ...
    pub dn: String,
    /// From the peer's HELLO at pairing time; UI icon only.
    #[serde(default)]
    pub dt: Option<String>,
    pub paired_at: u64,
    pub last_seen: u64,
    /// Exact `ip:port`, advisory: never establishes trust by itself.
    #[serde(default)]
...
impl TrustStore {
    ...
    /// `dt` and `addr` are hints only; neither participates in the trust decision.
    pub fn insert(
        &self,
        pubkey: &[u8; 32],
        dn: &str,
        ...
        // One address maps to at most one pinned identity, or a lookup resolves
        // to a stale key and hard-fails a peer that legitimately took it over.
        if let Some(a) = addr {
    ...
    /// No-op for unpinned keys, so an untrusted peer can never write here.
    ...
    /// Ambiguous matches resolve to `None`, never to an arbitrary key.
    ...
    /// Catches a HELLO claiming a pinned identity behind a different cert key
    /// (PROTOCOL.md §4.1.1).
    ...
    /// Ordered by `paired_at` then device id so the shell's list is stable.
